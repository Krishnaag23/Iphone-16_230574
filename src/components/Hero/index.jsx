...
const Hero = () => {
    const [videoSrc, setVideoSrc] = useState(window.innerWidth < 760 ? smv : hmv); // Set video source based on screen width
    const [isMobile, setIsMobile] = useState(window.innerWidth < 760); 

    const handleVideoSrcSet = () => {
        const isMobileView = window.innerWidth < 760;
        setVideoSrc(isMobileView ? smv : hmv); // Update to the correct video source
        setIsMobile(isMobileView ); // Update mobile state
    };
    ...
    useGSAP(() => {
        const textDelay = isMobile ? 4 : 12; // Set different delays for mobile
        ...
        <section className="w-full nav-height bg-black relative">
            <div className="h-full w-full bg-black flex flex-col items-center justify-center">
                ...
                <div id="cta" className="flex flex-col rounded-lg bg-black items-center opacity-0 transition-opacity duration-700">
                    <a href="#highlights" className="btn text-white font-bold">Get Your iPhone 16 Now!</a>
                    <p className="font-normal text-xl text-white">Starting at just ₹3329/month or ₹79900 one-time payment.</p>
                    <a href="." className="inline-block bg-blue-600 text-white font-semibold py-3 px-8 rounded-lg hover:bg-blue-700 transition-colors mt-4">
                        Discover More
                    </a>
                </div>
                <div id="cta-text" className="flex flex-col items-center text-white opacity-0 translate-y-20 mt-8 md:mt-16">
                    <div className="text-center px-4 md:px-10 md:w-3/5 w-full">
                        ...
                        <p className="text-lg mb-8">Experience unparalleled performance and innovative features that redefine mobile technology.</p>
                        <h2 className="text-xl md:text-3xl font-bold mb-4">Elevate Your Experience</h2>
                        <p className="text-lg mb-6">Join the mobile innovation revolution today!</p>
